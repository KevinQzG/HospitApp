...
describe('Specialty Model', () => {
  ...
  describe('Validation', () => {
    ...
    it('should throw error when name is missing', () => {
      const INVALID_SPECIALTY = new Specialty(undefined, '', TEST_SCHEDULE);
      expect(() => INVALID_SPECIALTY.validate()).toThrow('Missing required fields');
    });
  });
  ...
  describe('Partial Schedule Handling', () => {
    it('should handle undefined schedule days', () => {
      const MINIMAL_SPECIALTY = new Specialty(undefined, 'Minimal Specialty');
      
      expect(MINIMAL_SPECIALTY.getScheduleThursday()).toBeUndefined();
      expect(MINIMAL_SPECIALTY.toObject().schedule_friday).toBeUndefined();
    });
  });
});