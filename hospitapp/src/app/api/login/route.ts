...
const validateRequestBody = (body: LoginRequest): { success: boolean; error: string } => {
...
export async function POST(request: Request) {
    const userRepo = CONTAINER.get<UserRepositoryAdapter>(TYPES.UserRepositoryAdapter);
    ...
    try {
        ...
        if (user && user.comparePassword(body.password)) {
            ...
            response.headers.set("Set-Cookie", `session=${session}; Path=/; HttpOnly; SameSite=Strict; Max-Age=3600`);
