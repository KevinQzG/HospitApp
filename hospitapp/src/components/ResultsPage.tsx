...
const RESULT_ITEM = memo(({ item }: { item: IpsResponse }) => {
	...
	return (
		<Link
			...
			className={`block p-4 rounded-2xl transition-all duration-300 border h-40 flex flex-col ${promoted
		...
		>
			<div className="flex items-start space-x-3 flex-1">
				<div
					className={`p-2.5 rounded-xl flex-shrink-0 ${promoted
				...
				<div className="flex-1 min-w-0">
					...
					<div className="group flex items-center space-x-1.5 mt-1">
						<MapPin
							className={`w-4 h-4 ${promoted
									? "text-amber-500 dark:text-amber-400"
									: "text-blue-500 dark:text-blue-400"
								}`}
						/>
						<p
							className={`text-sm font-medium ${promoted
									? "text-amber-600 dark:text-amber-400"
									: "text-blue-600 dark:text-blue-400"
								}`}
						>
