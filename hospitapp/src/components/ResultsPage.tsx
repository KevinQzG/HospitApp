...
function ResultsDisplay({ specialties, eps }: SearchFormClientProps) {
  ...
  const applyFilters = (currentSortFields: SortField[]) => {
    ...
    const nonPromotedResults = filteredResults.filter(
      (item) => !isPromoted(item.promotion)
    );

    // Apply sorting only to non-promoted results
    if (currentSortFields.length > 0) {
      nonPromotedResults.sort((a, b) => {
        for (const sort of currentSortFields) {
          const aValue = a[sort.field as keyof IpsResponse];
          const bValue = b[sort.field as keyof IpsResponse];
    ...
    // Combine results with promoted IPS always at the beginning
    ...
    setPaginatedResults(combinedResults.slice(start, end));
    setTotalResults(combinedResults.length);
    setTotalPages(Math.ceil(combinedResults.length / pageSize));
  };
  ...
  const handleDirectionChange = (index: number, direction: number) => {
    ...
    // Apply filters immediately with new sort direction
    applyFilters(newSortFields);
  };
  ...
  const handleFieldChange = (index: number, field: string) => {
    ...
    currentParams.set("sorts", sorts);
    router.push(`/results?${currentParams.toString()}`, { scroll: false });
    // Apply filters immediately with new field
    applyFilters(newSortFields);
  };
  ...
  const removeSortField = (index: number) => {
    ...
    router.push(`/results?${currentParams.toString()}`, { scroll: false });
    // Apply filters immediately after removing field
    applyFilters(newSortFields);
  };
  ...
  useEffect(() => {
    const fetchFilteredResults = async () => {
      try {
        ...
        // Get current sort fields from URL
    ...
    // Only fetch results when necessary (new search, filter changes, or initial load)
    const shouldFetch =
      isNewSearch ||
      searchParams.has("specialties") ||
      searchParams.has("epsNames") ||
      searchParams.has("maxDistance") ||
      !allResults.length;

    if (userCoordinates !== null && shouldFetch) {
      fetchFilteredResults();
    }
  }, [searchParams, userCoordinates, isNewSearch, allResults.length]);
