...
const StarRating = ({ rating }: { rating: number }) => {
	...
	const formattedRating = Number.isInteger(rating)
		? rating
		: rating.toFixed(1); // Para el texto
...
function ResultsDisplay({ specialties, eps }: SearchFormClientProps) {
	...
	const handleFieldChange = (index: number, field: string) => {
		console.log("Changing sort field at index:", index, "to:", field);
	...
	const updateSortQuery = (newSortFields: SortField[]) => {
		...
		router.push(
			`/examples/ips-details/filter?${currentParams.toString()}`,
			{ scroll: false }
		);
	};
		...
		const fetchFilteredResults = async () => {
			try {
				...
				if (sortsParam) {
					...
					console.log("Parsed sorts from URL:", sorts);
					setSortFields(sorts); // Update sortFields state
				} else {
					console.log("No sorts in URL, using default");
					setSortFields([]);
				}
				...
				if (!response.ok)
					throw new Error(
						"No se pudieron obtener los resultados filtrados"
					);
					...
					<div className="flex items-center space-x-2">
						{sortFields.map((sortField, index) => (
							...
							>
								<select
									...
									className="p-1 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
								>
								...
								<button
									...
									className="p-1 text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 text-sm"
								>
									✕
								</button>
							</div>
						...
						{sortFields.length < sortOptions.length && (
							<button
								...
								className="p-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-sm"
							>
								Agregar Orden
							</button>
						)}
		...
		`;
				popupContent
					...
					?.addEventListener("click", () => {
						router.push(
							`/ips-details/${encodeURIComponent(item.name)}`
						);
					});
