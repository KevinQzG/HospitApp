...
export default function Header() {
  ...
  const _CHANGE_LANGUAGE = (lang: string) => {
    const _SELECT = document.querySelector(".goog-te-combo") as HTMLSelectElement;
    if (_SELECT) {
      _SELECT.value = lang;
            ...
            <nav className="hidden md:flex md:items-center md:space-x-6">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span className="notranslate">{_LANGUAGES[_LANGUAGE_INDEX]}</span>
                    ...
                    {_LANGUAGES.map((lang, index) => (
                      <button
                        ...
                        className={`block px-4 py-2 text-gray-700 hover:bg-gray-100 w-full text-center ${
                          _LANGUAGE_INDEX === index ? "font-bold text-blue-600" : ""
                        }`}
                      >
      ...
      {_IS_MOBILE && (
        <nav className="fixed bottom-0 left-0 w-full bg-white shadow-t-lg flex justify-around py-3 border-t border-gray-200 z-50">
          ...
          >
            ...
            <span className="text-xs font-bold notranslate">{_LANGUAGES[_LANGUAGE_INDEX]}</span>
          </button>
        </nav>
      )}
